...
export async function LocalizedProjectPage({ slug, locale }: { slug: string; locale: Locale }) {
  ...
  return <PublicShell overlay locale={locale} dark={luxury}><main className={luxury ? "lux-theme" : ""}>
    <section className={`relative text-white ${luxury ? "h-[100svh] min-h-[640px] md:min-h-[760px]" : "h-[90svh] min-h-[600px] md:min-h-[680px]"}`}>
    ...
    <section className={`grid border-b md:grid-cols-2 ${luxury ? "border-white/15" : "border-black/15"}`}>
    ...
    <section className={`container grid gap-12 border-y py-20 md:grid-cols-[1fr_2fr] md:py-28 ${luxury ? "border-white/15" : "border-black/15"}`}><p className="eyebrow">{d.information}</p><dl className="grid gap-x-8 sm:grid-cols-2">{[[d.location,project.address],[d.type,luxury ? (locale === "sr" ? "Lux rezidencija" : "Luxury residence") : project.category],[d.status,project.status === "ACTIVE" ? d.active : d.completed],[d.city,project.city]].map(([label,value]) => <div key={label} className={`border-t py-5 ${luxury ? "border-white/15" : "border-black/15"}`}><dt className="eyebrow mb-3 opacity-40">{label}</dt><dd className="break-words font-[var(--font-editorial)] text-3xl">{value}</dd></div>)}</dl></section>

    {project.floorPlans.length > 0 && <section className="container py-24 md:py-40"><div className="mb-14 text-center"><p className="eyebrow mb-5">{luxury ? (locale === "sr" ? "Rezidencija" : "The residence") : ""}</p><h2 className="section-title">{d.plans}</h2></div><div className="grid gap-10 md:grid-cols-2">{project.floorPlans.map((plan) => <article key={plan.id}><div className={`relative aspect-square overflow-hidden ${luxury ? "bg-[#191815]" : "bg-[#e5e2db]"}`}><Image src={plan.image} alt={plan.title} fill quality={95} sizes="(max-width: 900px) calc(100vw - 40px), 50vw" className="object-contain p-3 md:p-8" /></div><h3 className="mt-5 font-[var(--font-editorial)] text-3xl">{plan.title}</h3></article>)}</div></section>}

    <Units units={project.units} locale={locale} dark={luxury} />
    {related.length > 0 && <section className="container py-24 md:py-40"><div className={`mb-12 flex items-end justify-between border-t pt-5 ${luxury ? "border-white/20" : "border-black/20"}`}><h2 className="section-title">{project.status === "COMPLETED" ? d.previous : d.other}</h2><Link href={`/${locale}/izvedeni-projekti`} className="text-link">{d.viewAll}<span>→</span></Link></div><div className="grid gap-12 md:grid-cols-3 md:gap-8">{related.map((item) => <ProjectCard project={item} key={item.id} locale={locale} dark={luxury} />)}</div></section>}
  </main></PublicShell>;
}
