  ...
  return <>
    <div className="container">
      <div className="group relative aspect-[3/2] w-full overflow-hidden bg-[#181816] md:aspect-auto md:h-[72svh] md:min-h-[620px] md:max-h-[820px]" onTouchStart={(event) => touchStart.current = event.touches[0].clientX} onTouchEnd={(event) => { if (touchStart.current === null) return; const delta = event.changedTouches[0].clientX - touchStart.current; if (Math.abs(delta) > 45) (delta > 0 ? previous : next)(); touchStart.current = null; }}>
        ...
        <div className="absolute bottom-5 right-5 z-10 hidden text-[10px] tracking-[.18em] text-white md:block">{String(active + 1).padStart(2,"0")} / {String(images.length).padStart(2,"0")}</div>
      </div>
      <div className="mt-3 grid grid-cols-3 gap-2 md:mt-4 md:gap-3">{images.map((image, index) => <button type="button" key={image.id} onClick={() => setActive(index)} className={`relative aspect-[16/9] overflow-hidden bg-[#181816] transition ${index === active ? "opacity-100" : "opacity-45 hover:opacity-75"}`}><Image src={image.imagePath} alt="" fill quality={95} sizes="(max-width: 900px) 33vw, 500px" className={`object-contain ${image.type === "FLOOR_PLAN" ? "p-1" : ""}`} /><span className={`absolute inset-x-0 bottom-0 h-0.5 ${index === active ? "bg-[#bca16f]" : "bg-transparent"}`} /></button>)}</div>
    </div>
