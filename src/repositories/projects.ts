...
import type { FloorPlan, Project, ProjectCategory, ProjectImage, ProjectStatus, SiteSettings, TeamMember, Unit } from "@/types";
...
function baseProject(row: Row): Project {
  return {
    id: Number(row.id), category: String(row.category) as ProjectCategory, status: String(row.status) as ProjectStatus,
...
export async function getTeam(): Promise<TeamMember[]> {
  ...
  return (rows as Row[]).map((r) => ({ id: Number(r.id), name: String(r.name), role: String(r.role), photo: String(r.photo), description: r.description ? String(r.description) : undefined, sortOrder: Number(r.sortOrder), active: bool(r.active) }));
}
