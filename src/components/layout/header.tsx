...
export function Header({ projects, overlay = false, locale = "sr", dark = false }: { projects: Project[]; overlay?: boolean; locale?: Locale; dark?: boolean }) {
  ...
  const solid = scrolled || open || !overlay;const alternate=pathname.replace(/^\/(sr|en)/,locale==="sr"?"/en":"/sr");
  ...
  return <header onMouseLeave={()=>setActiveMenu(null)} className={`fixed inset-x-0 top-0 z-50 transition-all duration-500 ${solid ? dark ? "bg-[#11110f]/95 text-[#eee9df] border-b border-white/10 backdrop-blur-md" : "bg-[#f2f0eb]/95 text-[#171816] border-b border-black/10 backdrop-blur-md" : "text-white"}`}>
    <div className="container flex h-[var(--header-height)] items-center justify-between gap-8">
      <Link href={prefix} aria-label="STORMS početna" className="relative z-50 block w-[118px] shrink-0 md:w-[146px]">
      ...
      <nav className="hidden flex-1 items-center justify-end gap-[clamp(12px,1.25vw,26px)] lg:flex">
        {categories.map((cat) => <div className="relative py-8" key={cat.value} onMouseEnter={()=>setActiveMenu(cat.value)}>
          <Link className={`eyebrow whitespace-nowrap transition-opacity ${activeMenu&&activeMenu!==cat.value?"opacity-35":""}`} href={cat.href}>{cat.label}</Link>
        </div>)}
        <Link className="eyebrow whitespace-nowrap" href={`${prefix}/izvedeni-projekti`}>{d.nav.completed}</Link>
    ...
    <div className={`absolute inset-x-0 top-full hidden overflow-hidden border-t shadow-[0_40px_80px_rgba(0,0,0,.18)] transition-all duration-500 lg:block ${dark ? "border-white/10 bg-[#151512] text-white" : "border-black/10 bg-[#f2f0eb] text-[#171816]"} ${activeMenu?"pointer-events-auto translate-y-0 opacity-100":"pointer-events-none -translate-y-3 opacity-0"}`}>
